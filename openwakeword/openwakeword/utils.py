...
def worker_function(chunk, wakeword_models, inference_framework, prediction_function, kwargs, queue):
    filtered_model_kwargs = {key: value for key, value in kwargs.items()
    ...
    results = []
    for clip in chunk:
        func = getattr(model, prediction_function)
        filtered_pred_kwargs = {key: value for key, value in kwargs.items()
                                if key in func.__code__.co_varnames}
        results.append({clip: func(clip, **filtered_pred_kwargs)})
    queue.put(results)
...
def bulk_predict(
    ...
    for chunk in chunks:
        p = Process(
            ...
            args=(chunk, wakeword_models, inference_framework, prediction_function, kwargs, q)
        )
        ps.append(p)
